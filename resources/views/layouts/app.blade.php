        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            favoriteButtons.forEach(button => {
                button.addEventListener('click', function (e) {
                    ...
                    const icon = this.querySelector('i.bi-heart, i.bi-heart-fill');
                    
                    if(!icon) return;

                    fetch('{{ url("/favorites/toggle") }}', {
                        method: 'POST',
                        headers: {
                    ...
                    .then(data => {
                        if (data.status === 'added') {
                            ...
                            // If we are on the favorites page, we might want to hide the card immediately
                            if(window.location.pathname.includes('/favorites')) {
                                this.closest('.col-md-6').remove(); // remove the grid item
                            }
                        }
                    })
